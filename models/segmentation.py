...
def get_segmentation_model(backbone, feature_indices, feature_channels, arch):
    ...
    Returns:
        [type]: [description]
    """
    model = SegmentationEncoder(backbone, feature_indices, feature_channels, arch, diff=True)
    unet = UNet(model, feature_channels, 1, bilinear=True, concat_mult=1, dropout_rate=0.3,arch=arch)
...
def segmentation_model(backbone, feature_indices, feature_channels, arch):
    ...
    Returns:
        [type]: [description]
    """
    model = SegmentationEncoder1input(backbone, feature_indices, feature_channels, arch, diff=True)
    unet = UNet(model, feature_channels, 1, bilinear=True, concat_mult=1, dropout_rate=0.3,arch=arch)
    # unet = UNetSmall(model, feature_channels, 1, bilinear=True, concat_mult=1)
    return unet
