...
def load_backbone(args):
    ...
    if args.arch in vits.__dict__.keys():
        pretrain_model = vits.__dict__[args.arch](patch_size=args.patch_size, num_classes=args.image_size**2 * 3)
...
def define_G(args, init_type='normal', init_gain=0.02, gpu_ids=[]):
    if args.net_G == 'base_resnet18':
    ...
    elif args.net_G == 'base_transformer_other_backbones':
        print('load backbone')
        ...
        if 'vit' in args.arch:
            ...
            backbone = torchvision.models._utils.IntermediateLayerGetter(backbone, return_nodes)
        ...
        net = BASE_Transformer(input_nc=3, output_nc=2, token_len=4, backbone = backbone,
                             with_pos='learned')
...
class BASE_Transformer(torch.nn.Module):
    ...
    """
    def __init__(self, input_nc, output_nc, with_pos, backbone,
        ...
        self.conv_a = nn.Conv2d(384, self.token_len, kernel_size=1,
    ...
    def _extract_feature(self, image, n=8, return_h_w=False):
        # print('image.shape',image.shape)
        # print([name for name in self.backbone.named_modules()])
        # return_nodes = {
        #     'blocks': 'layer4',
        # }
        # backbone = torchvision.models._utils.IntermediateLayerGetter(self.backbone, return_nodes)
        # backbone.eval()
        out = backbone(x)
        # out = self.backbone.get_intermediate_layers(image, n=n)
        print(out)
        print('out.shape',out.shape)
        # for o in out:
        #     print(o.shape)
        out = out[:, 1:, :]  # we discard the [CLS] token
        # h, w = int(image.shape[2] / self.backbone.patch_embed.patch_size), int(image.shape[3] / self.backbone.patch_embed.patch_size)
    ...
    def forward(self, x1, x2):
        ...
        # forward other backbone
        # x1 = self.backbone(x1)
        # x2 = self.backbone(x2)
        x1 = self._extract_feature(x1)
        x2 = self._extract_feature(x2)
